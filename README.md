...
Worker сохраняет `ETag` sitemap и использует условные HTTP-запросы. Если N1 не публиковал изменений, сервер отвечает `304`, а XML не загружается и не разбирается — это удерживает cron внутри бесплатного CPU-лимита Cloudflare.
